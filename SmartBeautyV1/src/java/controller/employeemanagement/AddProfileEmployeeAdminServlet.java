 ...
 * @author admin
 */
public class AddProfileEmployeeAdminServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddProfileEmployeeAdminServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddProfileEmployeeAdminServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (fullname.isEmpty() || !fullname.matches("^[A-ZÀ-Ỵ][a-zà-ỹ]+( [A-ZÀ-Ỵ][a-zà-ỹ]+)+")) {
            ...
            request.getRequestDispatcher("addprofileemployeeadmin.jsp").forward(request, response);
            return;
        }
        ...
        if (!hireDate.isAfter(dob.plusYears(18)) || hireDate.isAfter(now)) {
            ...
            request.getRequestDispatcher("addprofileemployeeadmin.jsp" + account_id).forward(request, response);
            return;
        }
        // Validate Image URL
        if (image.isEmpty()) {
