...
public class UpdateprofilecustomerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        float weight;
        try {
            height = Float.parseFloat(request.getParameter("height").trim());
        } catch (NumberFormatException e) {
            request.setAttribute("error6", "Height must be valid numbers.");
            request.getRequestDispatcher("profile?account_id=" + account_id).forward(request, response);
            return;
        }
        try {
            weight = Float.parseFloat(request.getParameter("weight").trim());
        } catch (NumberFormatException e) {
            request.setAttribute("error7", "Weight must be valid numbers.");
            request.getRequestDispatcher("profile?account_id=" + account_id).forward(request, response);
            return;
        }
        ...
        response.sendRedirect("profile?account_id=" + account_id + "&success=Insert Profile Success");
    }
...
//        request.setAttribute("success", "Insert Profile Success");
